...
def update_parameters_velocity(parameters, gradients, epoch, velocity, svelocity, beta=0.9, epsilon=1e-8, learning_rate=0.1, decay_rate=0.01):
    '''
    ...
    for i in range(L):
        ...
        svelocity["dW" + str(i + 1)] = beta * svelocity["dW" + str(i + 1)] + (1 - beta) * np.power(gradients["dW" + str(i + 1)], 2)
        svelocity["db" + str(i + 1)] = beta * svelocity["db" + str(i + 1)] + (1 - beta) * np.power(gradients["db" + str(i + 1)], 2)

        parameters["W" + str(i + 1)] = parameters["W" + str(i + 1)] - learning_rate * gradients["dW" + str(i + 1)]/ np.sqrt(svelocity["dW" + str(i + 1)] + epsilon)
        parameters["b" + str(i + 1)] = parameters["b" + str(i + 1)] - learning_rate * gradients["db" + str(i + 1)]/ np.sqrt(svelocity["db" + str(i + 1)] + epsilon)
...
def multi_layer_network(X, Y, validation_data, validation_label, net_dims, num_iterations=500, learning_rate=0.1,
                        decay_rate=0.01, batch_size=10):
    '''
    ...
    for ii in range(num_iterations):
        for jj in range(batch_length):
            ...
            parameters, alpha, velocity, svelocity= update_parameters_velocity(parameters, gradients, ii, v,sv, beta, epsilon, learning_rate, decay_rate)
            # print parameters
            # if ii ==1:
            #     print "Iteration 2"
            #     print "dz : ",dZ
